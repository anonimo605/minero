...
const SatellitesSection = () => {
  ...
  const handlePurchase = async () => {
    ...
    if ((userOwnedCount + numQuantity) > selectedSatellite.purchaseLimit) {
        toast({ variant: "destructive", title: "Límite alcanzado", description: `No puedes comprar esta cantidad. El límite para esta plataforma es ${selectedSatellite.purchaseLimit} y ya posees ${userOwnedCount}.` });
        return;
    }
  ...
  return (
    ...
    <Card>
      <CardHeader>
        <CardTitle>Comprar Plataformas</CardTitle>
        <CardDescription>
          Usa tu saldo para comprar plataformas de minería y obtener rendimientos.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {satellites.length > 0 ? satellites.map((satellite) => {
            ...
            <div className="text-center text-muted-foreground py-8 col-span-full">
                <p>No hay plataformas disponibles para la compra en este momento.</p>
                <p className="text-sm">Por favor, pídele a un administrador que añada nuevas plataformas.</p>
            </div>
        )}
      </CardContent>
    </Card>
